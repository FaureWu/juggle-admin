...
const addEpic = (
  ...
  .ofType(ADD.START)
  .map(() => store.getState().tags.add.data.toJS())
  .map(data => ({
...
const editEpic = (
  ...
  .ofType(EDIT.START)
  .map(() => store.getState().tags.edit.data.toJS())
  .map(data => ({
    url: `${endpoints.editTag}/${data.key}`,
    headers: {
      token: window.localStorage.getItem('token'),
    },
    body: {
  ...
  .mergeMap(request => ajax(request)
    ...
    .map((response) => {
      if (response.code === 0) {
        store.dispatch(close());
        const data = store.getState().tags.edit.data.toJS();
...
const deleteEpic = (
  ...
  .mergeMap(request => ajax(request)
    ...
    .map(response =>
      response.code === 0 ?
      deleteSuccess(store.getState().tags.delete) :
