...
const getEpic = (
  ...
  .pluck('payload')
  .map(payload => ({
    url: `${endpoints.getArticles}/${payload.pos}/${payload.count}`,
    headers: {
...
const addEpic = (
  ...
  .ofType(ADD.START)
  .map(() => store.getState().articles.add.data.toJS())
  .map(data => ({
...
const editEpic = (
  ...
  .ofType(EDIT.START)
  .map(() => store.getState().articles.edit.data.toJS())
  .map(data => ({
    url: `${endpoints.editArticle}/${data.key}`,
    headers: {
      token: window.localStorage.getItem('token'),
    },
    body: {
  ...
  .mergeMap(request => ajax(request)
    ...
    .map((response) => {
      if (response.code === 0) {
        store.dispatch(close());
        const data = store.getState().articles.edit.data.toJS();
...
const deleteEpic = (
  ...
  .pluck('payload')
  .map(key => ({
    url: `${endpoints.deleteArticle}/${key}`,
    headers: {
      token: window.localStorage.getItem('token'),
    },
    method: 'DELETE',
