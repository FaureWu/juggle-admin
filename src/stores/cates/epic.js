...
const addEpic = (
  ...
  .ofType(ADD.START)
  .map(() => store.getState().cates.add.data.toJS())
  .map(data => ({
    url: endpoints.addCate,
    headers: {
      token: window.localStorage.getItem('token'),
    },
    body: {
...
const editEpic = (
  ...
  .ofType(EDIT.START)
  .map(() => store.getState().cates.edit.data.toJS())
  .map(data => ({
    url: `${endpoints.editCate}/${data.key}`,
    headers: {
      token: window.localStorage.getItem('token'),
    },
    body: {
  ...
  .mergeMap(request => ajax(request)
    ...
    .map((response) => {
      if (response.code === 0) {
        store.dispatch(close());
        const data = store.getState().cates.edit.data.toJS();
...
const deleteEpic = (
  ...
  .mergeMap(request => ajax(request)
    ...
    .map(response =>
      response.code === 0 ?
      deleteSuccess(store.getState().cates.delete) :
